...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            gt_rotations (Tensor): shape(bs, n_max_boxes, 6)
            gt_translations (Tensor): shape(bs, n_max_boxes, 3)
            mask_gt (Tensor): shape(bs, n_max_boxes, 1)

        Returns:
            target_labels (Tensor): shape(bs, num_total_anchors)
            target_bboxes (Tensor): shape(bs, num_total_anchors, 4)
            target_rotations (Tensor): shape(bs, num_total_anchors, 6)
            target_translations (Tensor): shape(bs, num_total_anchors, 3)
        ...
        if self.n_max_boxes == 0:
            return (
                ...
                torch.zeros_like(pd_rotations),
                torch.zeros_like(pd_translations),
    ...
    def _forward(
        ...
        (
            ...
            gt_labels, gt_bboxes, gt_rotations, gt_translations, target_gt_idx, fg_mask
        )
    ...
    def get_targets(
        ...
        """
        Compute target labels, bounding boxes, rotations, and translations for the positive anchors.

        Args:
            gt_labels (Tensor): Ground truth labels of shape (b, max_num_obj, 1).
            gt_bboxes (Tensor): Ground truth bounding boxes of shape (b, max_num_obj, 4).
            gt_rotations (Tensor): Ground truth rotations (6D) of shape (b, max_num_obj, 6).
            gt_translations (Tensor): Ground truth translations of shape (b, max_num_obj, 3).
            target_gt_idx (Tensor): Indices of the assigned ground truth for positive anchors.
            fg_mask (Tensor): Foreground mask of shape (b, h*w).
         ...
         Returns:
            (Tuple[Tensor, Tensor, Tensor]): A tuple containing the following tensors:
                ...
                - target_rotations (Tensor): Shape (b, h*w, 6), containing the target rotations
                                          for positive anchor points.
                - target_translations (Tensor): Shape (b, h*w, 3), containing the target translations
                                          for positive anchor points.
        ...
        target_rotations = gt_rotations.view(-1, gt_rotations.shape[-1])[target_gt_idx]
        target_translations = gt_translations.view(-1, gt_translations.shape[-1])[
            target_gt_idx
        ]
